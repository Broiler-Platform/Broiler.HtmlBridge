...
for name in on_disk:
    ...
    for path in files_under(directory, ".csproj"):
        try:
            ...
            packages = package_references(path)
        except ET.ParseError as exc:
            emit("error", "%s could not be parsed as XML (%s)."
                 % (path.replace(os.sep, "/"), exc))
            continue
        # Keep the historical metric name, but count both source and NuGet dependencies.
        # Switching a reference to a package must not look like reduced engine coupling.
        ...
        engine_project_refs += sum(
            ...
            if re.match(r"^Broiler\.(JavaScript|VM)(\.|$)", package, re.IGNORECASE))
...
while IFS=$'\t' read -r kind a b c d; do
  case "$kind" in
    ...
    record)
      ...
      ;;
    ok)
      printf '   ok  %-31s %4s references  %2s engine dependency refs  %3s eval sites\n' "$a" "$b" "$c" "$d"
      ;;
    scanned)
      scanned_projects="$a"
      scanned_files="$b"
