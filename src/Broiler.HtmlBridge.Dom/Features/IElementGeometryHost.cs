...
/// <c>scrollIntoView</c>, <c>scrollParent</c>). These genuinely read the live layout — every value comes
/// from the bridge's layout cache — so unlike the stateless feature modules this contract is deliberately
/// wide: it names the exact geometry surface the module depends on, so the callbacks do not reach into
/// arbitrary bridge internals.
