...
internal sealed class BroilerJsExoticObject : JSObject
{
    ...
    /// written against the object: <c>Array.prototype.map.call(list, …)</c> reads <c>length</c>
    ...
    /// answers no. <c>Object.keys</c>, <c>for…in</c> and spread ask the same way. Presence,
    ...
    /// mutation of its own to hook — what it reflects is the tree, and the read is the only moment it
    ...
    private uint Sync()
    {
        // RE-ENTRANCY GUARD, and it is not defensive programming — it closes a real stack overflow.
        // Materialising an index is `this[i] = …`, which the engine routes through SetIndexOnReceiver,
        ...
        // genuinely needs it when it is entered from outside — Object.keys asks for a descriptor per
        // key — and a guard here protects every entry point, including ones added later, instead of
