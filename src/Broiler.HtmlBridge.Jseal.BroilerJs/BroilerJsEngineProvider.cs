...
/// which are present, so the moment the two must agree is assembly load — which is exactly what a
...
/// by a particular point — a test that calls <see cref="JsEngineRegistry.Reset"/> and has to put the
...
public sealed class BroilerJsEngineProvider : IJsEngineProvider, IJsRealmAdoption
{
    ...
    /// find out. <see langword="null"/> — the default — means the question has not been asked, and
    ...
    /// checkout — it became true only with submodule patches 0010 (top-level-await codegen) and 0011
    ...
    /// honour and lets the host that already ran the probe publish the result here — one line,
    /// <c>BroilerJsEngineProvider.ModuleSupport = () =&gt; EngineModuleSupport.Available;</c> — so the
