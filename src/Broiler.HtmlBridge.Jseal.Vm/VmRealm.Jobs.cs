...
internal sealed partial class VmRealm
{
    ...
    /// The realm's own <c>Promise</c> — captured at creation, see <c>VmHostBridge.Promise</c> — is
    ...
    /// which is a difference between the two engines rather than a gap in this member — no JSEAL
    ...
    /// <c>new Promise(f)</c> has called <c>f</c> by the time it returns — the specification says so
    /// and the profile implements it that way — so the resolvers are in hand the moment
    ...
    /// ever enqueues, and a settle made from outside a step takes a turn — <c>#host-turn</c>, which
    /// runs the crossing and returns — rather than the profile's separate <c>#drain-jobs</c>. So
